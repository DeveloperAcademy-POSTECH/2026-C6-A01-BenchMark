...
import { boundedBody, checkOrigin, correctPassword, hash, HttpError, jsonError, makeSession, rateLimit, sessionCookie } from "@/lib/security";
...
export async function DELETE(request: Request) {
  try { checkOrigin(request); (await cookies()).delete(sessionCookie); return Response.json({ ok: true }); }
  catch (error) { return jsonError(error); }
}
