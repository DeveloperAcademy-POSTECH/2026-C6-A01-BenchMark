...
export function StoryReading({ story, donationButtonLabel }: { story?: Story; donationButtonLabel?: string }) {
  return <div className="story-reading"><ActivityTracker storyId={story?.id} /><div className="reading-brand"><img src="/brand/title.svg" alt="쉼, 펴" /></div><article className="story-detail">
    ...
    <Link className="button primary" href={story ? `/camera?from=${story.id}` : "/camera"}>캐릭터와 사진 찍기</Link>
    ...
    <Donation compact sourceStoryId={story?.id} buttonLabel={donationButtonLabel} /><Link className="text-link" href="/stories">다른 자리의 이야기도 만나보기 →</Link>
  </article></div>;
}
