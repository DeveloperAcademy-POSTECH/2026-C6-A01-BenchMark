...
export function ReservationForm({ sourceStoryId }: { sourceStoryId?: string }) {
  ...
  return <section className="reservation-page"><ActivityTracker storyId={sourceStoryId} /><a className="back-link" href={sourceStoryId ? `/stories/${sourceStoryId}` : "/"}>← 이야기로 돌아가기</a><div className="reservation-heading"><span className="eyebrow">다음 사람의 쉼을 위해</span><h1>기부 예약</h1><p>당신의 이야기도 한 자리 펼쳐주세요.<br />지금은 예약만 접수하며 결제는 진행되지 않아요.</p></div>
    <form ref={mainForm} onSubmit={askReason} onChange={() => { requestId.current = ""; }}><fieldset disabled={busy} className="reservation-fields">
      ...
      <fieldset className="reservation-question"><legend><span>04</span> 기부금액을 작성해주세요</legend><p id="reservation-amount-note" className="field-description amount-note">돗자리 기준 금액을 초과하는 금액은<br />‘포항공과대학교(POSTECH) 발전기금’으로 쓰입니다.</p><label className="amount-input"><span className="sr-only">기부금액</span><input name="amount" type="number" min={RESERVATION_MIN_AMOUNT} max={RESERVATION_MAX_AMOUNT} inputMode="numeric" step={1} placeholder="0" required aria-describedby="reservation-amount-note reservation-base-amounts" /><span className="amount-unit" aria-hidden="true">원</span></label><div id="reservation-base-amounts" className="base-amounts"><p>돗자리 기준금액</p><dl>{Object.entries(sizes).map(([key, size]) => <div key={key}><dt>{size.label}</dt><dd>{size.price.toLocaleString("ko-KR")}원</dd></div>)}</dl></div></fieldset>
