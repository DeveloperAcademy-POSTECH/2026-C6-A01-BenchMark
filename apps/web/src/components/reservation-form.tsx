...
import { byteLength, STORY_MAX_BYTES, sizes } from "@/lib/story";
import { reasons, paymentMethods, RESERVATION_MIN_AMOUNT, RESERVATION_MAX_AMOUNT, reservationDetailsInput, reservationInput } from "@/lib/reservation";
...
export function ReservationForm({ sourceStoryId }: { sourceStoryId?: string }) {
  ...
  return <section className="reservation-page"><ActivityTracker storyId={sourceStoryId} /><a className="back-link" href={sourceStoryId ? `/stories/${sourceStoryId}` : "/"}>← 이야기로 돌아가기</a><div className="reservation-heading"><span className="eyebrow">다음 사람의 쉼을 위해</span><h1>기부 예약</h1><p>당신의 이야기도 한 자리 펼쳐주세요.<br />지금은 예약만 접수하며 결제는 진행되지 않아요.</p></div>
    <form ref={mainForm} onSubmit={askReason} onChange={() => { requestId.current = ""; }}><fieldset disabled={busy} className="reservation-fields">
      ...
      <fieldset className="reservation-question"><legend><span>02</span> 성함과 이메일을 남겨주세요</legend><p id="reservation-contact-note" className="field-description">추후 정식 기부 운영 시, 남겨주신 이메일로 예약 내용의 진행 여부를 확인하는 메일을 보내드립니다.</p><label><span className="sr-only">성함</span><input name="displayName" autoComplete="name" maxLength={40} placeholder="성함" value={isAnonymous ? "익명" : displayName} onChange={(e) => setDisplayName(e.target.value)} disabled={isAnonymous} required={!isAnonymous} /></label><label className="anonymous-option"><input type="checkbox" name="isAnonymous" value="true" checked={isAnonymous} onChange={(e) => setIsAnonymous(e.target.checked)} />익명</label><label><span className="sr-only">이메일</span><input name="email" type="email" aria-describedby="reservation-contact-note" autoComplete="email" maxLength={254} placeholder="이메일" required /></label></fieldset>
      ...
      <fieldset className="reservation-question"><legend><span>04</span> 기부금액을 작성해주세요 <small>(10000원 이상, 자유금액 가능)</small></legend><p id="reservation-amount-note" className="field-description amount-note">돗자리 기준 금액을 초과하는 금액은<br />‘포항공과대학교(POSTECH) 발전기금’으로 쓰입니다.</p><label className="amount-input"><span className="sr-only">기부금액</span><input name="amount" value={amount} onChange={event => setAmount(event.target.value)} type="number" min={RESERVATION_MIN_AMOUNT} max={RESERVATION_MAX_AMOUNT} inputMode="numeric" step={1} placeholder="0" required aria-describedby="reservation-amount-note reservation-base-amounts" /><span className="amount-unit" aria-hidden="true">원</span></label><div id="reservation-base-amounts" className="base-amounts"><p>돗자리 기준금액</p><div className="amount-options" role="group" aria-label="돗자리 기준금액 선택">{Object.entries(sizes).map(([key, size]) => <button key={key} type="button" className="amount-option" aria-pressed={amount === String(size.price)} onClick={() => { setAmount(String(size.price)); requestId.current = ""; }}><span>{size.label}</span><strong>{size.price.toLocaleString("ko-KR")}원</strong><span className="amount-selection" aria-hidden="true">{amount === String(size.price) ? "✓ 선택됨" : "선택하기"}</span></button>)}</div></div></fieldset>
      {error && <p role="alert" className="error-message">{error}</p>}<button className="button primary reservation-submit" onClick={() => trackActivity("reservation_submit_click")} disabled={busy || byteLength(text) > STORY_MAX_BYTES}>{busy ? "예약을 접수하는 중…" : "기부 예약 접수하기"}</button><p className="reservation-footnote">운영진 확인 후 기부 진행 방법을 안내드려요.</p>
    </fieldset></form>
