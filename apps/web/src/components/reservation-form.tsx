...
export function ReservationForm({ sourceStoryId }: { sourceStoryId?: string }) {
  ...
  return <section className="reservation-page"><Link className="back-link" href={sourceStoryId ? `/stories/${sourceStoryId}` : "/"}>← 이야기로 돌아가기</Link><div className="reservation-heading"><span className="eyebrow">다음 사람의 쉼을 위해</span><h1>기부 예약</h1><p>당신의 이야기도 한 자리 펼쳐주세요.<br />지금은 예약만 접수하며 결제는 진행되지 않아요.</p></div>
    <form onSubmit={submit} onChange={() => { requestId.current = ""; }}><fieldset disabled={busy} className="reservation-fields">
      ...
      {error && <p role="alert" className="error-message">{error}</p>}<button className="button primary reservation-submit" disabled={busy || byteLength(text) > STORY_MAX_BYTES}>{busy ? "예약을 접수하는 중…" : "기부 예약 접수하기"}</button><p className="reservation-footnote">운영진 확인 후 기부 진행 방법을 안내드려요.</p>
    </fieldset></form>
  </section>;
}
