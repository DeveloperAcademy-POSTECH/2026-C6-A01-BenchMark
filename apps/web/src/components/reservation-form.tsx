...
export function ReservationForm({ sourceStoryId }: { sourceStoryId?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "예약을 접수하지 못했어요. 입력한 내용은 유지됩니다. 다시 시도해주세요."); }
    finally { locked.current = false; setBusy(false); }
  }
  if (done) return <section className="reservation-success"><span className="eyebrow">당신의 마음을 잘 받았어요</span><h1>기부 예약이 접수됐어요.</h1><p>남겨주신 이야기는 운영진이 확인할게요.<br />결제는 진행되지 않았으며,<br />기부 진행 방법은 남겨주신 휴대폰번호로 안내드릴게요.</p><Link href={sourceStoryId ? `/stories/${sourceStoryId}` : "/"} className="button primary">이야기로 돌아가기</Link></section>;
  return <section className="reservation-page"><Link className="back-link" href={sourceStoryId ? `/stories/${sourceStoryId}` : "/"}>← 이야기로 돌아가기</Link><div className="reservation-heading"><span className="eyebrow">다음 사람의 쉼을 위해</span><h1>기부 예약</h1><p>당신의 이야기도 한 자리 펼쳐주세요.<br />지금은 예약만 접수하며 결제는 진행되지 않아요.</p></div>
