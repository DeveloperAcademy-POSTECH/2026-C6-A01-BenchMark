...
import { sizes, type Story } from "@/lib/story";

function StoryEditor({ story, onClose }: { story?: Story; onClose: () => void }) {
  const router = useRouter();
  const [name, setName] = useState(story?.display_name || "");
  const [text, setText] = useState(story?.story || "");
  ...
  return <section className="editor" aria-labelledby="editor-title"><div className="section-heading"><h2 id="editor-title">{story ? "이야기 수정" : "새 이야기 등록"}</h2><button type="button" className="text-button" onClick={onClose} disabled={busy}>닫기</button></div><form onSubmit={save}><fieldset disabled={busy}>
    <div className="form-row"><label>돗자리 번호<input name="matNumber" type="number" min="1" max="10000" defaultValue={story?.mat_number} required /></label><label>돗자리 종류<select name="matSize" defaultValue={story?.mat_size || "small"}><option value="small">2~3인용 · 3,000원</option><option value="large">4~5인용 · 5,000원</option></select></label></div>
    <label>공개 이름<input name="displayName" value={name} onChange={(e) => setName(e.target.value)} maxLength={40} required /></label>
    <label>사진 한 장<input name="photo" type="file" accept="image/jpeg,image/png,image/webp" required={!story} onChange={(e) => { const file = e.target.files?.[0]; setPreview(file ? URL.createObjectURL(file) : ""); }} /><small>JPG, PNG, WebP · 최대 5MB. {story && "새 사진을 선택하지 않으면 기존 사진을 유지합니다."}</small></label>
    <label>이야기<textarea name="story" value={text} onChange={(e) => setText(e.target.value)} rows={4} required aria-describedby="story-length" /><small id="story-length" className={Array.from(text).length > 100 ? "error-message" : ""}>{Array.from(text).length} / 100자</small></label>
    ...
    <details className="preview-panel"><summary>공개 내용 미리보기</summary>{(preview || story) && <img src={preview || `/api/stories/${story!.id}/photo?v=${story!.revision}`} alt="공개할 사진 미리보기" />}<h3>{name || "공개 이름"}님의 이야기</h3><p>{text || "작성한 이야기가 여기에 표시됩니다."}</p></details>
    {error && <p className="error-message" role="alert">{error}</p>}<button className="button primary" disabled={busy || Array.from(text).length > 100}>{busy ? "처리 중…" : published ? "저장하고 공개" : "비공개로 저장"}</button>
...
export function AdminDashboard({ stories }: { stories: Story[] }) {
  ...
  return <div className="admin-page"><div className="section-heading"><div><span className="eyebrow">쉼, 펴 운영팀</span><h1>이야기 관리</h1></div><button className="text-button" onClick={logout}>로그아웃</button></div><p>네이버 폼 내용을 등록하고, 입금과 이야기를 확인한 뒤 공개해주세요.</p>
    ...
    <button className="button primary" onClick={() => setSelection("new")}>+ 새 이야기 등록</button>{error && <p role="alert" className="error-message">{error}</p>}
    {selection !== null && <StoryEditor key={selection === "new" ? "new" : selection.id} story={selection === "new" ? undefined : selection} onClose={() => setSelection(null)} />}
