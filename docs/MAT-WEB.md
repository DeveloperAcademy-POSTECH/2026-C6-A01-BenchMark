...
- `/reserve?from=:id`: 마음을 담은 한 줄·본문·사진 → 성함·이메일 → 희망 결제방법 → 3,000원/5,000원 선택 → 접수 버튼 → 예약 이유 팝업 → 최종 접수
- `/about`: 별도 소개 화면 없이 메인 `/`으로 영구 이동
- `/admin`: 페이지 하단 링크 → 공유 비밀번호 → 예약 접수함 및 이야기 관리

예약은 결제가 아니다. 결제방법은 무통장입금·간편결제·계좌이체 중 희망하는 방법을 기록하며 PG나 간편결제 API를 실행하지 않는다. 새 예약 금액은 3,000원 또는 5,000원만 선택할 수 있으며 클라이언트·서버에서 동일하게 검증한다. 기존 예약의 자유 입력 금액은 변경하지 않는다. 접수된 이야기는 운영진이 확인한다. 개인정보 수집 안내·동의 UI는 사용자 요청으로 생략했다.
