...
- `/reserve?from=:id`: 마음을 담은 한 줄·본문·사진 → 성함·이메일 → 희망 결제방법 → 10,000원 이상 기부금액 입력 → 접수 버튼 → 예약 이유 팝업 → 최종 접수
- `/about`: 별도 소개 화면 없이 메인 `/`으로 영구 이동
- `/admin`: 페이지 하단 링크 → 공유 비밀번호 → 예약 접수함 및 이야기 관리

예약은 결제가 아니다. 결제방법은 무통장입금·간편결제·계좌이체·기타 중 희망하는 방법을 기록하며 PG나 간편결제 API를 실행하지 않는다. 새 예약 금액은 10,000원 이상 원 단위 정수로 자유 입력하며 클라이언트·서버에서 동일하게 검증한다. PostgreSQL integer 저장 범위에 따라 최대 2,147,483,647원까지 허용한다. 기존 예약의 자유 입력 금액은 변경하지 않는다. 접수된 이야기는 운영진이 확인한다. 개인정보 수집 안내·동의 UI는 사용자 요청으로 생략했다.

신청 페이지에서 `익명`을 체크하면 성함 입력을 비활성화하고 서버에서 이름을 `익명`으로 저장한다. 체크 해제 시 입력했던 성함을 복원한다. 신규 신청은 이메일이 필수이며 전화번호를 수집하지 않는다. 관리자 이야기 등록 시 공개 이름에 `익명`이 채워진다. 예약 제목은 1~80자이며 기존 제목 없는 이야기는 이름으로 대체 제목을 표시한다. 본문은 UTF-8 기준 1000바이트 이내다. 한글은 보통 3바이트, 이모지는 4바이트다. 사진은 JPG/PNG/WebP 정지 이미지 한 장, 5MB 이하이며 실제 디코딩 후 긴 변 1600px 이하 JPEG로 저장한다. 방향을 보정하고 메타데이터를 제거한다.

## 운영

1. 관리자 접수함에서 예약 이메일·금액·이유·희망 결제방법·이야기·사진을 확인한다.
...
- 예약 이메일·기존 전화번호와 예약 이유·결제방법·금액은 관리자에게만 반환한다. 예약 사진 API도 관리자 인증을 요구한다.
...
- `178:595`를 바탕으로 한 예약 화면은 #15·#26 사용자 요청에 따라 마음을 담은 한 줄·본문·사진 → 이름·이메일 → 결제방법 → 금액 선택 순서로 변경했다. 예약 이유는 접수 버튼 이후 별도 팝업에서 묻는다.
...
사용자 결정에 따라 분석 동의 UI 없이 공개 페이지의 활동을 PostgreSQL에 수집한다. 별도 유료 분석 도구·세션 녹화는 사용하지 않는다. 관리자 `/admin`의 **페이지 활동 로그**에서 시작/종료 시각(종료 미포함), 이벤트, 페이지 경로, 스토리 UUID를 조합해 조회하고 **현재 조건으로 CSV 저장**할 수 있다. 조회는 최신순 100건씩이며 다음 페이지는 시각+이벤트 ID 커서로 이동한다. CSV는 현재 입력한 필터 전체 결과를 최대 5,000건까지 저장한다. 초과 시 기간/필터를 좁히도록 오류를 표시하고 일부만 저장하지 않는다. 일자 선택과 시작/종료 시각 입력, 화면 표시는 한국 시간(Asia/Seoul) 기준이다. `일자별`과 `기간·시간` 조회를 전환할 수 있고 `오늘`·`어제`로 빠르게 날짜를 선택한다. 기본 기간·시간 모드에서 시각을 비우면 최근 30일 전체를 조회한다. 일자별 모드에서는 시각 범위 입력을 숨기며, 해당 날짜 00:00 이상부터 다음 날짜 00:00 미만까지 조회한다. 기간·시간 모드로 돌아가면 입력했던 시각 범위를 다시 사용할 수 있다. 조회 결과 위에는 적용된 한국 시간 범위를 표시한다. CSV는 기존 UTC 열을 유지하고 `occurred_date_kst`, `occurred_at_kst` 열을 추가한다. 날짜 선택 시 파일명은 `page-activity-YYYY-MM-DD.csv`다. 조건을 변경하면 이전 조회 결과를 지워 화면·다음 페이지·CSV 조건 혼동을 방지한다. API의 `date=YYYY-MM-DD`는 기존 이벤트·경로·스토리 필터 및 커서와 조합 가능하며, `from`/`to`와 함께 전달하거나 존재하지 않는 날짜를 전달하면 400을 반환한다. API의 기존 `from`/`to` UTC 계약은 유지한다.
...
초기 입력 화면에서는 예약 이유를 표시하지 않는다. 마음을 담은 한 줄·스토리·사진·이름·이메일·결제방법·금액을 입력하고 **기부 예약 접수하기**를 누르면, 입력 검증을 거쳐 기존 예약 이유 선택지를 네이티브 dialog로 표시한다. 이유를 선택하고 **예약 접수 완료하기**를 눌러야 API 요청을 보낸다. 기타를 선택하면 추가 입력이 필수다.
...
## 신청 이야기 작성 안내와 이메일 연락처

신청 폼의 첫 질문은 “돗자리에 남길 이야기를 들려주세요”이며 작성 안내와 질문 3개를 제공한다. 제목 입력의 접근성 이름과 placeholder는 “마음을 담은 한 줄”로 표시한다. 제목 1~80자, 본문 UTF-8 1000바이트 제한은 유지한다.

신규 신청은 전화번호 대신 이메일을 필수로 받는다. 이메일은 앞뒤 공백 제거 후 형식·최대 254자를 검증하며 익명 신청에도 필요하다. 신규 전화번호 입력·검증·저장은 하지 않으며 구버전 클라이언트가 보내는 `phone` 값도 저장에서 제외한다. `007_reservation_contact.sql`은 phone의 NOT NULL만 제거하고 nullable email 컬럼을 추가한다. 기존 연락처를 삭제·변경하거나 이메일을 임의로 채우지 않는다. 기존 번호 형식 CHECK는 유지한다. 이메일은 관리자 접수함의 mailto 링크로 제공하며, 기존 전화번호도 관리자에게 보존한다. 연락처가 없는 경우 빈 링크를 만들지 않는다. 공개 이야기·분석 로그에는 이메일을 포함하지 않는다.

신규 예약 재시도는 이메일을 포함한 동일한 신청 내용으로 판단한다. 자동 이메일 발송이나 결제 연동을 추가하지 않는다. 완료 화면은 접수·미결제·운영진 확인 사실을 안내한다. 기존 예약에는 이메일이 없을 수 있으며, NULL phone이 생긴 뒤 NOT NULL 복원은 별도의 데이터 처리 결정이 필요하다.

## 추가 화면 피드백

성함·이메일 질문에는 추후 정식 기부 운영 시 예약 내용 진행 여부에 대한 확인메일을 보낸다는 안내를 표시한다. 이는 향후 운영 안내이며 자동 이메일 발송 기능은 포함하지 않는다.

이야기 제목·도입 문장·본문의 글자 크기를 각 화면 너비에서 기존 대비 15% 줄였다. 작은 보조 문구는 읽기 쉽도록 유지한다. 반응 숫자는 기존 빨간색 토큰으로 표시하고, 사용 안내서는 연한 파란 배경으로 이야기·기부 안내 영역과 구분한다.

결제방법에 ‘기타’를 추가하며 `008_reservation_payment_other.sql`로 DB 허용값도 확장한다. 기부금액은 최소 10,000원 자유 입력으로 받고 돗자리 기준금액(2~3인용 10,000원, 4~5인용 20,000원)을 안내한다. 기준금액 초과분의 ‘포항공과대학교(POSTECH) 발전기금’ 안내는 사용자 확정 문구다. 기존 예약 금액·연락처는 변경하지 않는다.

## 관리자 브라우저의 활동 로그 제외

관리자 로그인에 성공한 브라우저는 `mat_activity_excluded` 서명 쿠키로 이후 `/api/activity` 저장에서 제외한다. 쿠키는 HttpOnly·SameSite=Lax·production Secure이며 최대 400일 유지하고 제외 요청마다 갱신한다. 로그아웃과 8시간 인증 세션 만료 후에도 유지한다. 기존 유효 관리자 세션은 활동 요청 또는 로그아웃 시 자동 등록한다. 잘못된 비밀번호로는 등록되지 않으며, 제외 쿠키만으로 관리자 권한을 얻을 수 없다.

같은 사이트의 동일 브라우저·프로필에만 적용된다. 쿠키 삭제·차단·브라우저 자체 만료 정책, 시크릿 모드, 다른 브라우저, SESSION_SECRET 변경 뒤에는 다시 로그인해야 한다. 패치 전에 이미 만료·삭제된 로그인 이력은 복구할 수 없다. 로그인 전 저장된 기존 기록은 삭제하지 않는다. 브라우저의 활동 요청은 계속 발생할 수 있지만 서버에서 요청 본문 처리·DB 저장 전에 제외한다. 호스팅 서비스의 HTTP 접근 로그는 이 기능의 대상이 아니다.
